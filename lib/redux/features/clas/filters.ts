...
export const filterByLevel =
    (level: string): ClasFilterFn =>
    (clas) => {
        if (level === "all") return true;
        return clas.grade_level === level;
    };

export const filterByAllophone =
    (option: string): ClasFilterFn =>
    (clas) => {
...
export const composeFilters =
    (...filters: ClasFilterFn[]): ClasFilterFn =>
    (clas) =>
        filters.every((f) => f(clas));
