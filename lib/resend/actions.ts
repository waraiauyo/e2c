...
export async function sendEventCreatedNotification(
    ...
    try {
        ...
        console.log(
            `📧 [sendEventCreatedNotification] ${participants.length} participant(s) trouvé(s) pour l'événement ${eventId}`
        );
        ...
        for (let index = 0; index < participants.length; index++) {
            ...
            if (!participant.profile) {
                console.warn(
                    `⚠️  Participant ${index + 1}/${participants.length} (ID: ${participant.profile_id}) n'a pas de profil`
                );
            ...
            try {
                console.log(
                    `📤 Envoi email ${index + 1}/${participants.length} à ${participant.profile.email}...`
                );
                ...
                if (result.error) {
                    console.error(
                        `❌ Erreur Resend pour email ${index + 1}:`,
                        result.error
                    );
                    ...
                    if (result.error.name === "rate_limit_exceeded") {
                        console.log(
                            `⏳ Rate limit atteint, attente de 1 seconde et réessai...`
                        );
                        ...
                        if (retryResult.error) {
                            console.error(
                                `❌ Échec après retry pour email ${index + 1}`
                            );
                            results.push({
                                success: false,
                        ...
                        console.log(
                            `✅ Email ${index + 1}/${participants.length} envoyé après retry (ID: ${retryResult.data?.id})`
                        );
                        results.push({
                            success: true,
                ...
                console.log(
                    `✅ Email ${index + 1}/${participants.length} envoyé (ID: ${emailId})`
                );
                ...
                console.error(
                    `❌ Exception lors de l'envoi email ${index + 1}:`,
                    error
                );
                results.push({
                    success: false,
        ...
        console.log(
            `📊 Résultat envoi: ${succeeded}/${participants.length} réussi(s), ${failed} échec(s)`
        );
...
export async function sendEventUpdatedNotification(
    ...
    try {
        ...
        console.log(
            `📧 [sendEventUpdatedNotification] ${participants.length} participant(s) trouvé(s) pour l'événement ${eventId}`
        );
        ...
        for (let index = 0; index < participants.length; index++) {
            ...
            const html = eventUpdatedTemplate(
                recipientName,
                eventForTemplate,
                updaterName
            );
        ...
        console.error(
            "Erreur lors de l'envoi des notifications de modification:",
            error
        );
        return {
            success: false,
...
export async function sendEventDeletedNotification(
    ...
    try {
        ...
        console.log(
            `📧 [sendEventDeletedNotification] ${participantIds.length} participant(s) à notifier pour l'événement supprimé`
        );
        ...
        for (let index = 0; index < participantIds.length; index++) {
            ...
            try {
                ...
                if (!participantResult.profile) {
                    console.warn(
                        `⚠️  Participant ${index + 1}/${participantIds.length} (ID: ${participantId}) introuvable`
                    );
                    results.push({
                        success: false,
                ...
                const html = eventDeletedTemplate(
                    recipientName,
                    eventTitle,
                    eventDate,
                    deleterName
                ...
                console.log(
                    `📤 Envoi email ${index + 1}/${participantIds.length} à ${participant.email}...`
                );
                const result = await resend!.emails.send({
                    from: FROM_EMAIL,
                ...
                if (result.error) {
                    ...
                    if (result.error.name === "rate_limit_exceeded") {
                        ...
                        console.log(
                            `✅ Email ${index + 1}/${participantIds.length} envoyé après retry (ID: ${retryResult.data?.id})`
                        );
                        results.push({
                            success: true,
                            emailId: retryResult.data?.id,
                ...
                const emailId = result.data?.id;
                console.log(
                    `✅ Email ${index + 1}/${participantIds.length} envoyé (ID: ${emailId})`
                );
                results.push({ success: true, emailId });
        ...
        console.log(
            `📊 Résultat envoi: ${succeeded}/${participantIds.length} réussi(s), ${failed} échec(s)`
        );
