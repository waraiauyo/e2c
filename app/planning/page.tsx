...
import { FilterSidebar, type FilterContext } from "@/components/planning/sidebar/FilterSidebar";
import { ViewToggle, type ViewType } from "@/components/planning/shared/ViewToggle";
...
import { addWeeks, addDays, addMonths, formatDateLong } from "@/lib/planning/utils/dateUtils";
import type { Event } from "@/lib/planning/types";

export default function PlanningPage() {
    const dispatch = useAppDispatch();
    const { user, profile, isLoading: userLoading } = useAppSelector((state) => state.user);
    ...
    useEffect(() => {
        if (filterContext) {
            localStorage.setItem("planning-filter-context", JSON.stringify(filterContext));
        }
    }, [filterContext]);
    ...
    const filteredEvents = useMemo(() => {
        ...
        return events.filter((event) => {
            if (filterContext.type === "personal") {
                ...
                return event.owner_type === "personal" && event.owner_id === filterContext.id;
            } else {
                // Afficher uniquement les événements du CLAS sélectionné
                return event.owner_type === "clas" && event.owner_id === filterContext.id;
            }
        });
