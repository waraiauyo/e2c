...
import { sendAccountCreatedNotification } from "@/lib/resend/actions";
...
export async function createUserAction(data: z.infer<typeof createUserSchema>) {
    ...
    try {
        ...
        // Envoi de l'email de notification avec les identifiants
        const emailResult = await sendAccountCreatedNotification(
            email,
            firstName,
            password
        );
        ...
        if (!emailResult.success) {
            // Rollback : suppression de l'utilisateur si l'envoi échoue
...
export async function updateUserAction(
    ...
    try {
        ...
        return {
            success: false,
            error:
                error instanceof Error ? error.message : "Erreur mise à jour",
        };
    }
}
